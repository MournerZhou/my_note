[Python](https://so.csdn.net/so/search?q=Python&spm=1001.2101.3001.7020) 虽然支持多继承，但是 python 支持的多继承也是有限的。
...
**注意：上面代码里当在子类中通过父类名调用时，parent 被执行了 2 次**
 ...
 **注意：在上面模块中，当在子类中通过 super 调用父类方法时，parent 被执行了 1 次。**
